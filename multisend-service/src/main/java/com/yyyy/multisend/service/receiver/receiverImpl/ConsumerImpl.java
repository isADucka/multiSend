...
public class ConsumerImpl implements Consumer {
    ...
    public void comsumerMsg(MsgTask msgTask) {
        //调用线程池去分别执行
        //先拼接这个线程池的名称
        String poolName = msgTask.getReceiverType().getCode() + "" + msgTask.getMsgType().getType();
        poolMsg.setMsgTask(msgTask);
//        PoolHolder poolHolder=new PoolHolder();

        poolHolder.getPoolByName(poolName).execute(poolMsg);
//        System.out.println(poolHolder.getPoolByName(poolName));
//        System.out.println(PoolHolder.poolMap.get(poolName));
//        PoolHolder.poolMap.get(poolName).execute(poolMsg);
